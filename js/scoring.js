(function(root){
  ...
  function questions(a){
    const list=[{id:'main',eyebrow:'まずは、今の気分から',title:'一番楽しみたいのは？',hint:'いちばん近い気分をひとつ選んでね。',options:D.interests}];
    if(D.refinements[a.main]) list.push({id:'detail',eyebrow:'もう少しだけ、聞かせて',hint:'気になるものがなければ、こだわらなくても大丈夫。',...D.refinements[a.main]});
    return list.concat(D.commonQuestions.map(q=>q.id==='companion'?{...q,options:q.options.filter(o=>o.id!==a.main)}:q));
  }
